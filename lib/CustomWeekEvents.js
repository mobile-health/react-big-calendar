...
var DayEvents = function DayEvents(props) {
  ...
  var updateSizing = (0, _react.useCallback)(function () {
    if (events.length && !showAll) {
      ...
      console.log('num of children: Math.trunc((parentWidth - 12) / 26) = ', number);
      console.log('================');
  ...
  }, (visibleNumber > 0 || showAll) && events.map(function (item, idx) {
    var title = item.title,
      name = item.name,
      id = item.id,
      color = item.color;
    if (idx < visibleNumber || showAll) return /*#__PURE__*/_react.default.createElement("div", {
